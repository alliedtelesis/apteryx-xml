...
sch_node_child_next (sch_node *parent, sch_node *node)
{
    ...
    if (!sch_is_leaf (parent))
    {
        for (n = xml->children; n; n = n->next)
        {
            if (n != node || n->type != XML_ELEMENT_NODE)
            {
                continue;
            }
            return n->next;
...
sch_xml_to_gnode (sch_instance *instance, sch_node *schema, GNode *parent, xmlNode *xml, int depth)
{
    ...
    if (sch_is_list (schema)) {
        ...
        for (child = xmlFirstElementChild (xml); child; child = xmlNextElementSibling (child)) {
            if (g_strcmp0 ((const char *)child->name, key) == 0 &&
                strlen ((char *) xmlNodeGetContent (child)) > 0) {
                if (xmlNextElementSibling (child)) {
                    DEBUG ("%*s%s\n", (depth + 1) * 2, " ", (const char *)child->name);
                    APTERYX_NODE (node, g_strdup ((const char *)child->name));
                }
                continue;
            }
            GNode *cn = sch_xml_to_gnode (instance, sch_node_child_first (schema), NULL, child, depth + 1);
            if (!cn) {
        ...
        if (g_strcmp0 ((const char *)xmlFirstElementChild (xml)->name, key) == 0 &&
            strlen ((char *) xmlNodeGetContent (xmlFirstElementChild (xml))) > 0) {
            xml = xmlFirstElementChild (xml);
